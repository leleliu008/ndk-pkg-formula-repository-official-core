...
build() {
    gmakew -C "$PACKAGE_BSCRIPT_DIR" clean &&
    gmakew -C "$PACKAGE_BSCRIPT_DIR" CC="$CC" CFLAGS="'$CFLAGS $CPPFLAGS'" LDFLAGS="'$LDFLAGS'" AR="$AR" RANLIB="$RANLIB" uname_S=Linux uname_M="$TARGET_OS_ARCH" BUILD_TLS=yes USE_JEMALLOC=no &&
    gmakew -C "$PACKAGE_BSCRIPT_DIR" install PREFIX="$TARGET_INSTALL_DIR"
}
